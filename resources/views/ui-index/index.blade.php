...
@section('content')
                    ...
                    @foreach ($categories as $category)
                        <a class="category-card" href="{{ route('index.categories', $category->category_id) }}">
                            <div class="category-image"
                                style="background-image: url('/uploads/{{ $category->cover_image }}');">
                            </div>
                            ...
                            <div class="swiper-slide">
                                <div class="product-card">
                                    <div class="product-image">
                                        ...
                                        @endphp
                                        @if ($activeDiscount)
                                            <div class="product-discount sale-badge">SALE
                                                -{{ number_format($activeDiscount->discount_percent, 0) }}%</div>
                                        @else
                                    ...
                                    <a class="product-info" href="{{ route('product.details', $product->product_id) }}">
                                        ...
                                        @php
                                            ...
                                            $coreSpecsData = [
                                                'CPU' => $specsMap->first(
                                                    fn($v, $k) => Str::contains(strtolower($k), [
                                                        'cpu',
                                                        'chip',
                                                        'vi xử lý',
                                                    ]),
                                                ...
                                                'RAM' => $specsMap->first(
                                                    fn($v, $k) => Str::contains(strtolower($k), 'ram'),
                                                ),
                                                'GPU' => $specsMap->first(
                                                    fn($v, $k) => Str::contains(strtolower($k), [
                                                        'gpu',
                                            ...
                                            @foreach ($coreSpecsData as $name => $value)
                                                @if ($value)
                                                    <div class="spec-grid-item">
                                                        <img src="{{ $specIconFiles[$name] }}"
                                                            alt="{{ $name }} icon" class="spec-grid-icon">
                                        ...
                                        <div class="product-price">
                                            @if ($activeDiscount)
                                                <span
                                                    class="sale-price"><?= number_format($activeDiscount->sale_price, 0, ',', '.') ?>₫</span>
                                                <span
                    ...
                    @foreach ($posts as $post)
                        <article class="news-item">
                            <div class="news-content">
                                ...
                                <span class="arrow">
                                    <svg fill="currentColor" viewBox="0 0 23 24" xmlns="http://www.w3.org/2000/svg"
                                        class="w-5">
    ...
    <!-- Chatbot Bubble -->
    <script src='https://cdn.jotfor.ms/agent/embedjs/019abf67745377df8d515ddced74eb40a02a/embed.js'></script>
